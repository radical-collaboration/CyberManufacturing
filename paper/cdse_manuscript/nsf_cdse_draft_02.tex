...
pilot for scaling studies from 1 processor to 128 for the PBM and upto 256 cores for the DEM. Using 
...
OMP)to model the bulk processes occuring during the granulation process.
...
\caption{This shows the isometric view of the impeller inisde the L\"{o}dige CoriMix CM5 continuous high shear granulator casing.}
...
movement of the particles while (\subref{fig:mthds_dem_shear_element}) aids to direct the partilces to the wall 
...
\textcolor{cyan}{The PBM code was initially developed using empirically derived kernels which did not use DEM collision data and resulted in a stand alone model which is breifly discussed here and in more detail in the supplemental materials section.}
%The code utilized static arrays, whose size was allocated on initiation and then the code was parallelized using the same
%technique as dicussed above.
\textcolor{cyan}{
The code was written in c using only staticly allocated linear arrays. The PBM parallelization algorithm in this work was developed using this stand alone PBM for
 conventional clusters having two CPUs per node.}
  %This code was compiled using the intel/15.0.2 and mvapich2/2.1 compilers. The code was tested on
%Stampede1 supercomputer for the configuration of the cores varying from the 1 to 128 cores. 
\textcolor{cyan}{The code was tested on the Stampede1 supercomputer using 1 to 128 cores. All jobs were compiled using the intel/15.0.2 and mvapich2/2.1 compilers, and all jobs were run using the "tacc affinity" option to account for socket affinites.}
%The speedup obtained for this simulation 
...
\textcolor{cyan}{The test simulations resulted in a run time of 222 seconds using 1 core and a run time of just 6 seconds when using 128 cores. More detailed performance stuides of the stand alone model are available in the supplemental materials section and will be breifly touched on in the results section of this manuscript.} 
\textcolor{cyan}{Since the hybrid parallel technique used for the stand alone model showed good performance it was also used for the coupled PBM.} 


The DEM informed PBM code was rebuilt in c++ using dynamically sized multi-dimensaional arrays to accommodate the different numbers of particles present in the DEM simulations.
The coupled PBM code developed uses the Standard Template Library containers and their features available in C++ 11 and later.
It requires intel v17.0.2 (gcc 5.3+)  or later for compilation. Since this module was not present on the Stampede, the execution of the 
PBM was done on the newer Stampede2 supercomputer. Each of the compute nodes of the cluster consists of an Intel Xeon Phi 7250 
...
\item $\Re$ integrates for each first solid bin as $(i1)$ $\int_{0}^{s_2(OMP 0)}$, $(i2)$ $\int_{s_2(OMP 0)}^{s_2(OMP 1)}$, $(i3)$ 
$\int_{s_2(OMP 1)}^{s_2(OMP 2)}$   and so on are divided into smaller integrals  $\int_{i_1low_n}^{i_1up_n}$, $\int_{i_2low_n}
...
\caption{The integration of aggregation rate distribured among OMP processes. This figure indicates the use of 4 OMP processes, the 
...
\caption{The speedup achieved in the DEM simulations of 2 mm partilces. There is a linear increase upto 16 cores, then as the parallel 
efficiency decreases, plateauing of the speedip is observed.}
...
\caption{The distrbution of time taken by each component of the DEM simulation with varying number of cores for the 2mm particles. It 
...
Figure \ref{fig:rslts_PBM_timing_studies} shows that the program scales really well to about 32 cores but then, 
...
\textcolor{red}{8 OMP in each MPI or 8 OMP total becuase earlier it is stated that 4 OMP was the best?}
...
%\textcolor{cyan}{speed up section}
...
\textcolor{cyan}{Figure \ref{fig:rslts_PBM_speed_up} depicts the speed up achieved by the hybrid parallel PBM code. When run using 1 to 16 cores only MPI is used to parallelize the PBM. The speed up in the range of core counts from 1 to 16 is nearly linear. This behavior is due to the spatial division of the PBM that is done using MPI. Since there are 16 compartments being used in this model, using 16 MPI processes means that each process only has to solve one spatial bin. Beyond 16 cores OMP is implemented along side MPI and adds about 10\% more performance up to 32 cores. After 32 cores the performance does not improve. 
}

\textcolor{cyan}{The sections of code OMP is used to parallelize have a large number of  nested loops which have been known to be difficult to parallelize \citep{He2016} using the native C++'s OMP libraries. Additionally dynamic multi-dimensional arrays and STL containers are extremely difficult to operate on using shared memory parallel methods like OMP due to memory locking when pointers are accessed which often times forces threads to access memory in serial which greatly reduces possible performance gains.}  

%\textcolor{cyan}{The stand alone PBM that was initally developed and tested did not use multi-dimensional arrays and was able to achieve a speed up of 37 times, which can be seen in figure \ref{fig:rslts_sa_pbm_speedup} in supplemental materials \ref{sec:supp_sa_pbm_performance}. It was also found that the stand alone model achieved the best performance using 8 OMP threads per MPI process on Stampede1 since each CPU had 8 physical cores. This would suggest it is a However the stand alone PBM did not have to read in and process large amounts of DEM data.}

\textcolor{cyan}{The stand alone PBM (programmed in c) that was initally developed did not use multi-dimensional arrays, since it did not have to read in DEM data, and was found to achieve optimial speed up on Stampede 1 using 8 OMP threads per MPI process (Stampede1 has 8 cores per CPU). The stand alone model was able to achieve a speed up of 37 times, which can be seen in figure \ref{fig:rslts_sa_pbm_speedup} in supplemental materials \ref{sec:supp_sa_pbm_performance}. Since the stand alone PBM performanced best with 8 OMP threads per MPI process it suggests that there are indeed issues with c++ parallelizition using OMP in the coupled model. However since the coupled model was developed from the stand alone model it is likely that with reasonable modificaitons to the coupled model and running it on a cluster like Stampede1 similar performance is achievable. For this reason the performance data here may be somewhat of a conservative estimate of what the coupled PBM could be capable of. For this work though the coupled model was more than fast enough since DEM was by far the rate limiting step.}

%\textcolor{cyan}{Since the stand alone PBM was able to achieve a 37 times speed up it is likely that the coupled PBM could also reach similar performance levels if it was run on a different a cluster with architecture similar to Stampede1 and arrays could be linearized and accessed in parallel. For this reason the coupled PBM performance in figure \ref{fig:rslts_PBM_speed_up} should be take as a somewhat concervative estimate of what it could be possible to achieve with minor tweaks to the program.} 
...
%\textcolor{cyan}{parallel efficinecy}
...
\textcolor{cyan}{ Figure \ref{fig:rslts_PBM_parallel_efficiency} shows the parallel efficinecy of the coupled PBM running on 1 to 128 cores for the same jobs shown in figure \ref{fig:rslts_PBM_speed_up}. It can be seen that efficiency decreases fairly quickly from 1 to 32 cores and then decreases more slowly after that. When cores are added in the 1-16 range a new MPI process is being utilized and addes extra communication over head resulting in a noticeable decrease in efficiency. After 16 cores the number of MPI processes remains the same and OMP processes are added. From 16 to 32 cores OMP is introduced and some overhead is involved in doing so. From 32 to 64 cores the OMP threads improve perforance and thus maintian efficiency. However when  the extra OMP processes being used to run 128 cores are introduced they interfere with each other due to the issues discussed previously.}
\textcolor{cyan}{The stand alone PBM had better parallel efficinecy since it did not have the c++ related issues with OMP. The parallel efficincey data for the stand alone model can be seen in the supplemental materials section. Without the OMP threading issues the stand alone model was able to achieve an efficicney of 0.289 at 128 cores. Again since the coupled model is built on the same framework it is very likely that this same performace could be achieved using the coupled model with the appropriate modifications and using a differenct cluster. However as stated earlier since the coupled PBM was so much faster than the DEM simulations it was not necessary for this work to futher enhance the efficiency of the coupled PBM, which even with threading issues performed well.}  

\begin{figure}[H]
\centering
\includegraphics[scale=1]{rslsts_PBM_efficiency.pdf}
\caption{The parallel efficiency for the hybrid (MPI + OMP) parallelized coupled PBM code.}
...
a more convinient method to perform simulations which have large number of different submissions are required.
(\textbf{Currently a placeholder})}
...
\input{supp_nsf_cdse_draft02.tex}
