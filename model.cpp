...
//**************** MPI load Array Start *******************
   ...
   if(mpi_id==0){ printf("nc=%d and mpi_proc=%d\n",NUMBEROFCOMPARTMENTS,num_mpi); }
   int load_xi=0;
   //int load_pp=0;

   ierr = MPI_Barrier(MPI_COMM_WORLD);

   if(print==1) {printf("we now start to fill load with values\n");}
   for(int i = 0; i < NUMBEROFCOMPARTMENTS; i++)
   {
    load[load_xi]+=1; //=load[load_xi]+1;
    if(print_load==1 && mpi_id==0) {printf("load[%d]=%d\n",load_xi,load[load_xi]); }
     load_xi++;
    if(load_xi==num_mpi) {load_xi=0;}
   }

   ierr = MPI_Barrier(MPI_COMM_WORLD);

   if(print==1){printf("start 2nd part of load calcs\n");}

   for(int i = 0; i < num_mpi+1; i++)
    {
     load_rev[num_mpi-i]=load[i];
     if(print_load==1 && mpi_id==0)  {    printf("load_rev[%d]=%d\n",num_mpi-i,load_rev[num_mpi-i]);  }
    }
   ...
   for(int i=0; i<num_mpi+1; i++) {
    ...
    if(print_load==1)
     {
      //if(mpi_id==0) { printf("load_rev[%d]=%d\n",i,load_rev[i]); }
      if(mpi_id==0) { printf("load[%d]=%d\n",i,load[i]); }
     }
    }

    ierr = MPI_Barrier(MPI_COMM_WORLD);
   /*dist lower and upper chunks to cores*/
   if(print==1) {printf("now distribute core_low and core_up  to each process\n");}
   ...
   for(int i=0 ; i<num_mpi ; i++)
    ...
    if(mpi_id==i)
     { core_low=load[mpi_id] /*core_low=load[id]+1*/;
       ...
       printf("my name=%d, core_low=%d, core_up=%d\n",mpi_id,core_low,core_up);
     }
    }
    ...
    if(mpi_id==0)
    {
     /* recv   */
       ...
       /*  loop to put correct array values into masters copy */
        ...
        //{
         /*c=i;*/
         for( int c=load[i]; c < load[i+1]; c++) //int c=0; c < NUMBEROFCOMPARTMENTS; c++)
          { //int c=load[i]; c < load[i+1]; c++) {
...
if(mpi_id != 0 )
{
         for(int c=0; c < NUMBEROFCOMPARTMENTS; c++)  //int c=0; c < NUMBEROFCOMPARTMENTS; c++)
          { //int c=load[i]; c < load[i+1]; c++) {
          for (int i0 = 0; i0 < NUMBEROFFIRSTSOLIDBINS; i0++)
           {
           for (int ix = 0; ix < NUMBEROFSECONDSOLIDBINS; ix++)
...
// ******************************* END time step Calcs/checks ***************************************************
    ...
    cout << endl << "nTimeSteps = " << nTimeSteps << endl << endl;
