...
These processes are widely popular as these partculate products have great advantages over 
...
physics of these procesess is poorly understood \citep{Rogers2013}. As a results industries 
that rely on these processes, especially pharmecutical, have to use expensive 
...
run on.
\par High performance computing (HPC) is the utilization of supercomputers and parallel processing
...
Python which provides task level parallelisation, to help develop an accurate model which can be 
...
\par The particle-particle collisions are not always elastic, thus there is a need for models for the 
...
in the velocity during the plastic deformation during a collision was accounted by \cite{Cundall1979} and 
...
\par High Performance Computing clusters are a collection of nodes interconnected by a high speed 
...
a speed up of about 98\% using 128 cores over the serial code. \cite{Prakash2013a},\cite{Prakash2013b} 
used the inbuilt Parallel Computation Toolbox (PCT) in Matlab to parallelize their PBM on lower number 
of cores, but this faced the shortcomings of Matlab's internal processing and could not achieve the 
speed improvements of parallelistion of a program if it were written in a native programming language 
like C or FORTRAN. 
...
process to the Xeon phi co-processors was added. The initail timing studies were performed on Stampede 
...
\par  The impeller consists of a cylindrical shaft of length 370 mm and diameter 68 mm with four 
...
\caption{a) Shows the side view of the impeller. b) and c) show the front and back views of the impeller 
which correspond to the inlet and outlet ends respectively.}
%\caption{hello yuk}
...
\par The simulation data was collected after ever 50,000 time steps (5$times 10^{-3}$ secoonds) 
for the visulation of the particles inside the shell, further post processing . The collisions 
...
%\par where, $G_{shear}$ is the shear rate exerted by the impeller on the granules. $R(s_1,s_2,x)$ is 
...
\par One MPI process was used per CPU socket and one OMP thread was used per CPU core, as 
...
the cores of the the multi-core CPU the MPI process is bound too. The OMP threads divide up and 
...
step. 
\par Since the PBM code developed used the std libraries present in C++ 11 and later. It requires gcc 5.3 or
...
("Knights Landing") which has 68 cores on a single socket clocked at 1.4 GHz, with 96 GB of DDR4 RAM. Each of 
...
size 1 mm and the simulation was carried out for 0.5 seconds of granulation time. The initial timing studies
...
maximum of 2.1 mm. \\
%Since such a large range of diameters are being covered, the number of particles of the smaller bins '
%(Bins 1 - 4) of the granulator will be higher than the number of particles in the larger bin sizes. \\ 
...
lower diameter particles. Figure \ref{fig:rslts_PBM_d50_plots}(\subref{fig:100s}) shows the distribution 
...
\par The RADICAL pilot (RP) being a pilot job system it can bypass the need of waiting on the long scheduler
...
\centering
\includegraphics[scale=0.5]{rslts_PBM_2mm_validation.pdf}
\caption{The ratio of rate of formation to the rate of depletion due to aggraetion 
...
\par Since the platforms used for initial development and testing of the DEM and PBM respectively were different, 
...
\par The times for the individual DEM and PBM simulations were added to determine the total amount time taken to 
...
the PBM execution completed. The Figure \textbf{refer bar plot here} shows a comparison of the total times of simulation 
...
the times of the simulation can be accounted to the communication required for the pilot realise that the DEM simulation 
has compeleted and link the data from the DEM to the PBM executable before it is executable. But, this communication is 
...
\section{Conclusions}
\par A simple uni-directional DEM+PBM coupled study for a high shear granulator has been presented. DEM simulations were 
...
was then used in the multi-dimensional PBM which was developed with 2 solid bins. Both these executables were parallelised 
to help them utilize the multi-core hardware of a cluster. The DEM was parallelised using MPI where as the developed PBM used 
MPI as well as OMP for a faster execution. The speedup acheived for the DEM simulation was about 12 times using 64 cores, 
where as the PBM acheived a speedup of 14 times when 16 cores were used. RP was utilized to execute the simulations for lower 
wait times.
\par 
\section*{References} 
