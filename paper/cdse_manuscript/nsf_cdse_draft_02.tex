...
Since the PBM code developed used the Standard Template Library containers and their features available in C++ 11 and later. It requires 
intel v15.0.2 (gcc 5.3+)  or later for compilation. Since this module was not present on the Stampede, the execution of the PBM 
was done on
the newer Stampede2 supercomputer. Each of the compute node of the cluster consists of Intel Xeon Phi 7250 
(\textquotedblleft Knights Landing\textquotedblright) which has 68 cores on a single socket clocked at 1.4 GHz, with 96 GB of DDR4 RAM.
...
\item $\Re$ integrates for each first solid bin as $(i1)$ $\int_{0}^{s_2(OMP 0)}$, $(i2)$ $\int_{s_2(OMP 0)}^{s_2(OMP 1)}$, $(i3)$ $\int_{s_2(OMP 1)}^{s_2(OMP 2)}$   and so on are divided into smaller integrals  $\int_{i_1low_n}^{i_1up_n}$, $\int_{i_2low_n}^{i_2up_n}$, $\int_{i_3low_n}^{i_3up_n}$ upto 16 bins 
...
\caption{The integration of aggregation rate distribured among OMP processes. This figure indicates the use of 4 OMP processes, the configuration that gave us the best performance.}
...
\begin{table}[ht]
\caption{Comparison of time taken for the DEM simulations using 128 and 256 core due to different spatial decomposition configurations.}
...
\caption{The variation in the amount of time taken for the DEM simulation of 2 mm particles as a function of number of cores. The speed increase on increasing the number of cores from 1 to 16 is higher than the speed improvements observed from increase of cores from 16 to 256.}
...
\caption{The speedup achieved in the DEM simulations of 2 mm partilces. There is a linear increase upto 16 cores, then as the parallel efficiency decreases, plateauing of the speedip is observed.}
...
\caption{The distrbution of time taken by each component of the DEM simulation with varying number of cores for the 2mm particles. It can be observed that the percentage of time in communication between the processes is the highest due to the rotation of the impeller at high speeds.}
