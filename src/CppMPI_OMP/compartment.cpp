...
CompartmentOut performCompartmentCalculations(PreviousCompartmentIn prevCompIn, CompartmentIn compartmentIn, CompartmentDEMIn compartmentDEMIn, double time, double timeStep)
{
    ...
    array<double, NUMBEROFSECONDSOLIDBINS> vss = compartmentIn.vss;
    ...
    int s = 0, ss = 0, s1 = 0, ss1 = 0, s2 = 0, ss2 = 0, a = 0, b = 0;

#pragma omp parallel private(omp_id, s, ss, s1, ss1, s2, ss2, a, b)
    {
        omp_id = omp_get_thread_num();
        //cout << "me mpi_id= "<< mpi_id << " omp_id= "<<omp_id<<" compSeg= "<<compSeg << endl;

        if (omp_id == 0) //#pragma omp master   // omp master call 1  OOOOOOOOOOOOOOOOOOMMMMMMMMMMMMMMMMMMMMMMMPPPPPPPPPPPPPPPPPPPPPP
        {
...
#pragma omp for // pragma omp for call 1 start
        ...
        } // if omp_id==0 call 2 end //pragma master call 2 end  ************************** OMP ********************************
...
#pragma omp for
        ...
        if (omp_id == 0) // if omp_id==0 call 3    ***********************************OMP*****************************
        {
        ...
        } //if omp_id == 0 call 3 // omp master call 3 end

    } // this is brace that ends pragma omp parallel
