...
problem across many CPUs which can work together to solve the problem faster.\csnote{Giannis can rewrite this paragraph}. 
...
clusters. 
 ...
 Development of 4-Dimensional, reduced order DEM informed PBM that is parallelized using hybrid 
techniques (MPI + OMP)to model the bulk processes occurring during the granulation process.
 Perform DEM simulations on LAMMPS Improved for general granular and granular heat transfer 
simulations (LIGGGHTS) to  model the micro-mechanics of the L\"{o}dige CoriMix 
CM5 high shear granulator. 
 Unidirectionally couple DEM to PBM using Radical Pilot, which is a framework developed in
...
LAMMPS Improved for general granular and granular heat transfer simulations (LIGGGHTS) is an open-source software used 
to perform DEM simulations. This package natively 
...
LAMMPS Improved for general granular and granular heat transfer simulations (LIGGGHTS) v3.60(\cite{Kloss2012}) 
developed by DCS computing was used for all the simulation performed 
