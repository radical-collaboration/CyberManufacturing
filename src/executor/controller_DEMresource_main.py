...
class controller_DEMresource_main(object):
    def __init__(self, init_timestep, types, liggghts_output_path, pbm_timestep):
    ...
    def main(self):
        ...
        if (flag == 1):
            ...
            with open('PBM_input.in', 'w') as ipt:
                ...
                ipt.write("\nAggregationKernelConstant %s"%(str(1.0e-1)))
                ipt.write("\nBreakageKernelConstant %s"%(str(5.0e-2)))
                ipt.write("\nConsolidationConstant %s"%(str(1.0e-40)))
                ipt.write("\nInitialPorosity %f"%(0.5))
...
abcd = controller_DEMresource_main(int(sys.argv[1]), int(sys.argv[2]), sys.argv[3], float(sys.argv[4]))
abcd.main()
