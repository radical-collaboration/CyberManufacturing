...
this developed framework, the granulation process has been modeled 
...
Discrete Element Method (DEM)~\citep{Cundall1979} simulations model the many particle-particle 
force interactions in these particulate systems using newtons equations. The 
particle-level simulation data is then used to help predict the bulk properties
of these system \citep{Hancock2011}. However because DEMs model all particle-particle 
interactions they are very time consuming to solve especially for very large 
particulate systems which prevents them from being practial to use in many situations. 

%Recently, Discrete Element Method (DEM)~\citep{Cundall1979} simulations
...
Population balance models (PBMs) are a faster but less accurate way of 
modeling these particulate sytems. They are semi-mechanistic in nature, meaning 
they use population averages and probability to model bulk behavior but still 
seek to capture some of the micro-phenomena of particle-particle interactions 
using correlations or empirically developed kernels to approximate those
interactions. Even though PBMs are much faster than DEMs, a detailed PBM can
...
\citep{Barrasso2013}. Also even very detailed PBMs can still have difficulties 
capturing the micro-scale phenomena that are crucial to predicting accurate 
dynamics of particulate processes.
...
Thus, to complement the strengths of each of these modeling techniques they
are coupled to produce a model which is more accurate than a PBM but much more 
efficient than DEM. The typical work flow of such a
DEM-PBM coupled model involves using a short DEM simulation to capture the
particle-particle level interactions of the system then that data is
...
This DEM-PBM method is more accurate than a PBM simulation alone but runs much
faster than a complete DEM simulation. Despite the performance benefits of
these coupled DEM-PBM models they still take unacceptably long to solve. In
the past High-Performance Computing (HPC) has been used to speed up computationally
...
PBM simulations~\citep{Bettencourt2017}~\citep{Prakash2013a}~\citep{Gunawan2008}.
...
In this work a unidirectional DEM-PBM coupled model was developed to efficiently 
predict the behavor of a wet-granulation process. HPC methods were implemented to use modern
computing clusters to solve the model as quickly as possible. 
The goal of this work was to solve a model, which can accurately simulate the physics of
these particuate processes, in a time frame that would make it feasable for wide 
scale use in industry and acadamia. 
...
models to design these processes in silico. %developing a mathematical model 
%to represent the rate process.

\subsection{Modeling}

The paradigm shift of the pharmaceutical industry towards QbD emphasizes the need
for a more accurate models. As the industry switches to continuous prodution methods
over tradtional batch methods there will be a dire need for accurate 
predictive controllers that can midigate any loss of API in processing which will
...
This method tracks the movement of each particle and records all forces on the particle
such as particle-geometry collisions, particle-particle-collisions, gravitational forces,
and select force-fields that apply~\citep{Barrasso2015cerd}.

\subsubsection{Discrete Element Modeling (DEM)}
 Discrete Element Method is a simulation technique used to model the
behavior of systems of particles. This method tracks the movement of each particle
 and records all forces on the particle such as particle-geometry collisions, 
 particle-particle collisions, gravitational forces, and select force-fields that 
 apply~\citep{Barrasso2015cerd}. This model is based on the Newton's laws of
...
particle, and $F_{i,net}$ represents the net force on the particle. Forces on the
...
\subsubsection{Population Balance Model (PBM)}
    ...
    a granulator change as rate processes such as aggregation and breakage reshape particles~\citep{Barrasso2013}. A general form of population balance model is shown in Equation~\ref{eqn:bkgd_pbm_general}.
...
describe the process dynamics with more accuracy an a PBM but more efficently than DEM. However, the computation
required to solved these coupled models when using a large number of particles becomes very intensive and time consuming, hence the need for HPC methods to help solve these models more quickly.
...
Since the many cores of a CPU can communicate directly through memory they can communicate using shared 
memory methods. This is commonly done using Open Multi-Processing (OMP) which uses threading to 
parallelize the cores. The cores can then communicate implicitly in most cases, directly through memory. 
This also allows processes to share common data automatically reducing communication overhead and reduces the overall 
memory footprint required for computation.

When many CPUs are being used in parallel they cannot communicate with each other directly through memory 
which results in much slower communication and therefore shared memory methods like OMP should not be used. 
This type of system where processes cannot communicate directly through memory is called a 
distributed memory system. Message passing interface ( MPI) is commonly used to enable communciation of the processes 
of a distrubted memroy system. MPI allows the processes to communciate using explicit  message passing methods. 
MPI will operate each process as a discrete entity with its own private copy of each variable it needs 
for computation which is how it avoids memory retrieval slowdowns when using many CPUs. For optimial parallel performance a 
programmer needs to consider the architecture of the cluster they are using and the features of the applicatons they are using
 such as MPI and OMP ~\citep{Adhianto2007}.  
...
Toolbox (PCT) in MATLAB~\citep{pctMatlab} to parallelize their PBM using less than 16 cores, 
but faced short comings due to the overhead of MATLAB software and therefore could not achieve
the same level of performance as a program written in C/C++ or FORTRAN.  
...
particles and the walls of the shell resulting in better size control of the granules. There were two sets of 
...
particles radii had twice the volumne of the size class before them. The radii range of the distributed size simulation 
was 1mm to 3mm. The difference in the mechanics of these two simulations is discussed in the Section~\ref{sec:Results}. 
...
calculated as:~\citep{Chaturbedi2017} \fbnote{should this citation come before the :?} 
...
where, the aggregation kernel, $\beta(s_1,s_2, s_1',s_2',x)$ is expressed as a function of the collision 
rate coefficient ($C$) and probability that a collision will result in agglomeration($\psi$)~\citep{ingram2004}
and is shown below: 
...
DEM simulation. A recent study shows that collision frequency depends on the Particle Size Distribution (PSD) as well~\citep{sen2014} \fbnote{this line seems shoehorned in here. Need to explicitly state why it relates to this equation right now in the paper. Might make more sense to bring this study up before you talk about using mono and distributed particle size systems. This study would give reason for why both might be informative. }. Collision rate coefficient for every time step can be expressed as:
...
clusters being used. To parallelize the model in a way which
...
\fbnote{bec pseudo code changed this section needs to be rewritten since 
$F_{t,c}$ does not refer to any part of the psuedo code anymore. We will haveto work on this} 
...
\gpnote{Yes, it is in the same format as the CCGrid paper. Much better than before. I suggest that you reference the algorithm. Using "below" I expect the algorithm near this paragraph, but it is in the Appendix.} Then each
...
containers and their features available in \textit{C++} 11 and later. \gpnote{I do not think you have to
give a reason this specific for the selected resource. Stampede2 supports the requirements of the
software, and as a result it is the selected resource for the experiments} It
...
suggests a performance solution. Doesn't make sense to me.} \csnote{edited structure of the paragraph} The
...
MATLAB \gpnote{needs citation} script was run on the dump files obtained from the simulation and it
...
Thus, when a large number of these simulations need to be run, the could uploaded 
as a single pilot job using RADICAL pilot reducing scheduler wait times on individual 
jobs. \jhanote{It is unclear what the
benefit is!}\csnote{tried to address the benefit elaborately} An extension of this work would be to use RP to schedule multiple
...
This parallelization technique is decried as an algorithm in Algorithm \ref{alg:parpbm}.
