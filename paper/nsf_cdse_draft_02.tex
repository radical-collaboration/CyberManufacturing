...
particluate processes. 
 Thus, to complement the limitations of each of these modelling methods they are coupled to 
...
predicted process dynamics of the granulation process using such multi-physics models.
 Initally, \cite{ingram2005} coupled PBM and DEM using two different mutliscale frameworks and 
...
\ref{table:mthds_dem_input}.
 The simulation data was collected after ever 50,000 time steps (5$times 10^{-3}$ secoonds) 
...
%	     \textcolor{red}{citation?}    
 ...
 where, $F(s_1,s_2,x)$ is the number of particles with an API volume of $s_1$ and an excipient 
...
non-linear binning was used.
 Once the PBM had been discretized (compartmentalized/binned) a finite differences method was 
...
efficiently solved using a shared memory system  
 Pseudo code is presented below illustrating how the calculations are distributed and carried out 
...
step. 
 Since the PBM code developed used the std libraries present in C++ 11 and later. It requires gcc 5.3 or
...
depletion due to aggregation helps us monitor whether the PBM satifies the conservation of mass. 
...
Since the degree of aggregation that has occured is very low, thus there is a jump in the number of 
