...
class liggghts_input_creator(object):
    ...
    def __init__(self, init_timestep, final_timestep, min_dia, max_dia, types_of_particles, total_flow_rate, solid_density,final):
    ...
    def main_writer(self):
        ...
        if (top > 0 and min_d > 0 and max_d >= min_d and fin_ts > in_ts):
            ...
            in_file.write("neigh_modify       delay 0 check yes page 1000000 one 30000\n")
            in_file.write("timestep           0.0000005\n")
...
a = liggghts_input_creator(int(sys.argv[1]),int(sys.argv[2]),0.002,0.002,16,15,500,final=bool(int(sys.argv[3])))
#in_file.write("\n")