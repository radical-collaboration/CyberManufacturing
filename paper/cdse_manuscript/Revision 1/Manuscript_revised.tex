...
to PBM was performed using middle-ware components (RADICAL-Pilot) that did not
...
method, implementations and middle-ware enable the modeling of high shear wet
...
Though highly detailed PBMs \citep{Immanuel2005}\citep{Poon2008} do take into account
various dimensions, populating, averaging and contain semi-mechanistic kernels, these PBMs still
can have difficulties in capturing the micro-scale phenomena that are crucial
to predicting accurate dynamics of particulate processes \citep{Christofedis2007}.
...
This model is based on the Newton's laws of motion as shown \citep{cundall1992}: \\
\begin{align}
...
physics of the system especially velocity profiles \citep{gantt2005}
but are not directly applicable to process-level simulations.

These granular particles usually have a visco-elastic type of behavior, 
thus the particle-particle and particle-wall collisions are modeled using a
spring-dashpot rheological model \citep{Dosta2018}. The Hertz-Mindlin 
contact model \citep{Mindlin1953} has been used in this work and it 
has been verified in earlier studies \citep{gantt2006}\citep{hassanpour2013}.
...
processes such as aggregation and breakage reshape particles~\citep{Barrasso2013}. 
A general form of population balance model is shown :
...
are the rate of formation and depletion due various phenomena occurring in granulation. 
...
to build a mechanistic model for the PBM. \citep{bouffard2012} evaluated spatial transfer in 
a compartmentalised PBM using DEM data, while \cite{Goldschmidt2003} solved a PBM using DEM by 
...
\subsection{Computer Architecture and Parallel Applications}
...
\begin{figure}
\centering
\includegraphics[scale=0.15]{Figure_1.pdf}
...
shown in Figure~\ref{fig:mthds_dem_impeller}. There are three different blade elements on the 
...
\begin{figure}
\centering
\includegraphics[scale=0.15]{Figure_2.pdf}
...
\label{fig:mthds_dem_impeller}
\end{figure}    

\begin{figure}
\begin{subfigure}{.3\textwidth}
\centering
\includegraphics[scale=0.075]{Figure_3a.pdf}	      
\caption{Feed element}
\label{fig:mthds_dem_feed_element}
\end{subfigure}%
\begin{subfigure}{.3\textwidth}
\centering
\includegraphics[scale=0.075]{Figure_3b.pdf}
\caption{Exit element}
\label{fig:mthds_dem_exit_element}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
\centering
\includegraphics[scale=0.075]{Figure_3c.pdf}
...
of a distribution 
of sizes. The particle radii chosen for mono-sized simulation varied from 0.59 mm to 2 mm, consecutive 
...
\hline
Young's Modulus of particles  & $8 \times 10^{6}$ & N~m\textsuperscript{-2}\\
Young's Modulus of Geometry  & $1 \times 10^{9}$ & N~m\textsuperscript{-2}\\
Poisson's Ratio & $0.2$ & $-$\\
Coefficient of restitution (constant for all collisions) & $0.4$ & $-$\\
Coefficient of static friction & $0.5$ & $-$\\
Coefficient of rolling friction  & $0.2$ & $-$\\
Density of the granules & $500$ & kg~m\textsuperscript{-3}\\
\hline
\end{tabular}
\end{center}
\end{table}
...
\label{sec:pbm_model}
The population balance equation used in this work is expressed as 
\citep{ramkrishna2000}:
\begin{align}
...
The rate of change of liquid volume for a given time in each particle is calculated using: 
...
liquid addition.
Similarly, the rate of change of gas volume for a given time is calculated using: 
...
($\psi$) in Equation~\ref{eqn:mthds_pbm_beta_kernal} can be determined by:
...
Velocity in axial direction & $v_{axial}$ & $1$ & m~s\textsuperscript{-1}\\
Velocity in radial direction & $v_{radial}$ & $1$ & m~s\textsuperscript{-1}\\
...
\label{mthds_DEM_postProcessing}
Post processing of data obtained from the DEM simulations was done using MATLAB. 
The first test run on the output data was to determine if the simulation had 
reached steady-state. The mass inside the granulator was found out by averaging 
it over 5 time steps and then compared to mass inside the granulator after 
every 10000 time steps (about 5$\times 10^{-4}$ sec) with a tolerance of 
10\%. If the mass was found to be constant for most of the iterations, it was 
considered to be at steady state. Another test to determine steady state was 
to monitor the number of collision inside the granulator. 
The visualization of the simulation data was done by running the LIGGGHTS 
post processing (LPP) script over the dump files to convert them into STL files. 
These files were then loaded in to Paraview~\citep{paraview2017} for a graphical 
representation of the simulation. It can be seen that the number of collision 
start to oscillate around a mean value. The number of collisions were then 
plotted and steady state time was determined. A precautionary script was also 
run to determine that no particles were lost due to overlap 
...
RADICAL-Pilot~\cite{rp-jsspp18} defines a number of abstractions to describe resources, and
...
The DEM data was used to determine the steady state micro-mechanics of the powder mixture 
inside the high shear granulator. These simulations were also checked for particle loss 
as mentioned in Section \ref{mthds_DEM_postProcessing}. Physical quantities obtained 
from the DEM simulation like velocity, collisions were then used in the PBM simulation 
to give it a more mechanistic nature. Figure \ref{fig:mthds_couplingNoRP} represents 
the flow of data in the coupling process, where each of the DEM and PBM job was submitted 
manually to the supercomputer.
...
\centering
\includegraphics[scale=0.5]{Figure_5a.pdf}
\caption{}
\label{fig:mthds_couplingNoRP}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.5]{Figure_5b.pdf}
\caption{}
\label{fig:mthds_couplingwithRP}
\end{subfigure}
\caption{Flow charts representing differences in between the coupling of the DEM and PBM 
simulations manually and using RP respectively.}
\end{figure}
 

RADICAL-Pilot~\citep{rp-jsspp18} is utilized to execute and monitor DEM and PBM simulations.
...
description. A pictorial representation of the data flow using RP can be found in Figure 
\ref{fig:mthds_couplingwithRP}.
...
simulations with different 3-D partition configuration were carried out. 
The comparison of the simulation times with the new configurations are shown in
...
DEM simulation times were compared using a mono-disperse particles distribution of 2 mm. 
The times plotted in
...
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.4]{Figure_6a.pdf}
\caption{}
\label{fig:rslts_DEM_2mm_timing}
\end{subfigure}\hfill
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.55]{Figure_6b.pdf}
\caption{}
...
\begin{equation}
Speedup = \frac{Wall \ Clock \ Time}{Parallel \ Wall \ Clock \ Time}\label{eqn:rslts_DEM_Speed_Up}
\end{equation}
...
\begin{equation}
\textit{Parallel Efficiency} = \frac{Serial \ Wall \ Clock \ Time}{Parallel \ Wall \ Clock \ Time \times n}
...
compared to the simulations with mono-sized particles of 1.59 mm and 2 mm, though the mean size of 
particles in the distribution was 2 mm. The default profile provided by LIGGGHTS indicated that the time 
...
\begin{figure}
\centering
\includegraphics[scale=0.7]{Figure_7.pdf}
\caption{Time taken for a 10 second DEM simulation for radii ranging from
 0.63 mm to 2 mm and a particle size distribution ranging from 1mm - 3mm. The speed improvements 
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{Figure_8.pdf}
\caption{The distribution of time taken by each component of the DEM 
simulation with varying number of cores for the 2 mm particles. It 
...
\includegraphics[scale=0.45]{Figure_9a.pdf}
\caption{}	
\label{fig:30s}
\end{subfigure}\hfill
\begin{subfigure}{.5\textwidth}

\includegraphics[scale=0.6]{Figure_9b.pdf}
\caption{}
\label{fig:50s}
\end{subfigure}
\begin{subfigure}{.5\textwidth}

\includegraphics[scale=0.45]{Figure_9c.pdf}
\caption{}
\label{fig:75s}
\end{subfigure}\hfill
\begin{subfigure}{.5\textwidth}

\includegraphics[scale=0.45]{Figure_9d.pdf}
\caption{}
\label{fig:100s}
\end{subfigure}
...
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.5]{Figure_11a.pdf}
\caption{}
\label{fig:rslts_PBM_speed_up}
\end{subfigure}\hfill
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.5]{Figure_11b.pdf}
\caption{}
\label{fig:rslts_PBM_parallel_efficiency}
\end{subfigure}

\caption{(a) The speedup achieved by the hybrid parallel implementation of the PBM program.
...
Two simulations from the study were compared, the Coupled DEM and PBM simulation of the 2 mm 
...
It can be seen from Figure~\ref{fig:rslts_DEM_timing_studies} that the 2 mm mono-sized particle had a speed
...
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.5]{Figure_12a.pdf}
\caption{}
\label{fig:rslts_PBM_2mm_d50}
\end{subfigure}\hfill
\begin{subfigure}{.45\textwidth}
\includegraphics[scale=0.4]{Figure_12b.pdf}
\caption{}
\label{fig:rslts_PBM_psd_d50}
\end{subfigure}
\caption{(a) D\textsubscript{50} of the particles obtained after 100 s of granulation time (25 s of 
mixing and 75 s of liquid addition) for the 2 mm mono-sized particle DEM simulation. (b) D\textsubscript{50} of the particles obtained after 100 s of granulation time (25 s of mixing and 75 s of liquid addition) for the distributed particle size DEM simulation. The trend observed was similar to the trend 
...
With resources allocated, the pilot can execute multiple simulations at once.
...
because the used version 0.47 of RADICAL-Pilot~\citep{rp-jsspp18} did not distinguish between 
...
\caption{A comparison of times taken by 2 individual simulations (DEM and PBM) (without 
including queue time) to the  time taken by each simulation in the RP job
of the RADICAL-Pilot. A small communication time overhead can be seen
...
\noindent Source code and input scripts for reproduction of these simulations can be found at:\\
