...
%Meshlab to exclude those pieces or re-mesh those individual pieces into better shapes with less 
...
The PBM code was initially developed using empirically derived kernels which did not use DEM collision 
data which is briefly discussed here and in more detail in the supplemental materials section.
The initial code was written in C using only pre-allocated static linear arrays. The development of the aforementioned 
PBM parallelization algorithm was done using this PBM. It was parallelized for a conventional cluster node having two 
sockets. The code was tested on the Stampede1 supercomputer using 1 to 128 cores. Compiler settings 
used were similar to the LIGGGHTS compilation and the PBM was run using the "tacc affinity" option with 
ibrun, so only 1 MPI process was allocated to each socket. The hybrid parallel technique in this model showed good 
performance, so this approach was also used for the coupled PBM model.


The DEM informed PBM code was rebuilt in c++ using dynamically sized multi-dimensional arrays to accommodate the 
different numbers of particles present in the DEM simulations.
The coupled PBM code developed uses the Standard Template Library containers and their features available in C++ 11 and 
later. It requires intel v17.0.2 (gcc 5.3+)  or later for compilation. Since this module was not present on the Stampede, 
the execution of the  PBM was done on the newer Stampede2 supercomputer. Each of the compute nodes of the cluster 
consists of an Intel Xeon Phi 7250 
(\textquotedblleft Knights Landing\textquotedblright) which has 68 cores on a single socket clocked at 1.4 GHz, with 96 
GB of DDR4 
...
can be seen when the MPI cores used are increased from 1 to 16 cores the speed up achieved also 
increases. 
...
%\textcolor{cyan}{The stand alone PBM that was initially developed and tested did not use multi-dimensional arrays and was able to achieve a speed up of 37 times, which can be seen in figure \ref{fig:rslts_sa_pbm_speedup} in supplemental materials \ref{sec:supp_sa_pbm_performance}. It was also found that the stand alone model achieved the best performance using 8 OMP threads per MPI process on Stampede1 since each CPU had 8 physical cores. This would suggest it is a However the stand alone PBM did not have to read in and process large amounts of DEM data.}

\textcolor{red}{Need to addresss this para later}\textcolor{cyan}{The stand alone PBM  that was initially developed was found to achieve optimal speed 
up on Stampede1 using 8 OMP threads per MPI process. This model was able to achieve a speed up of 37, 
which can be seen in figure \ref{fig:rslts_sa_pbm_speedup} in supplemental section 
\ref{sec:supp_sa_pbm_performance}. Since the stand alone PBM performance 
best with 8 OMP threads per MPI process it suggests that there are indeed issues with C++ 
parallelization using OMP in the coupled model. However since the coupled model was developed 
from the stand alone model it is likely that with reasonable modifications to the coupled model 
and running it on a cluster like Stampede1 similar performance is achievable. For this reason the 
performance data here may be somewhat of a conservative estimate of what the coupled PBM could 
be capable of. For this work though the coupled model was more than fast enough since DEM was by far the rate limiting step.}

%\textcolor{cyan}{Since the stand alone PBM was able to achieve a 37 times speed up it is likely that the coupled PBM could also reach similar performance levels if it was run on a different a cluster with architecture similar to Stampede1 and arrays could be linearized and accessed in parallel. For this reason the coupled PBM performance in figure \ref{fig:rslts_PBM_speed_up} should be take as a somewhat conservative estimate of what it could be possible to achieve with minor tweaks to the program.} 
...
decreases as the number of cores used increase. The fall in efficiency initially up till 8 cores is 
steep as the communication time in between the MPI process decreases the efficiency, deviating 
its value from the ideal of 1 to about 0.4. But with the increase in the MPI processes to 16 and then the 
...
% Figure \ref{fig:rslts_PBM_parallel_efficiency} shows the parallel efficiency of the 
%coupled PBM running on 1 to 128 cores for the same jobs shown in figure \ref{fig:rslts_PBM_speed_up}. 
%It can be seen that efficiency decreases fairly quickly from 1 to 32 cores and then decreases 
%more slowly after that. When cores are added in the 1-16 range a new MPI process is being 
%utilized and added extra communication over head resulting in a noticeable decrease in efficiency. 
%After 16 cores the number of MPI processes remains the same and OMP processes are added. From 16 
%to 32 cores OMP is introduced and some overhead is involved in doing so. From 32 to 64 cores the 
%OMP threads improve performance and thus maintain efficiency. 

\textcolor{red}{However when  the extra OMP 
processes being used to run 128 cores are introduced they interfere with each other due to 
the issues discussed previously.The stand alone PBM had better parallel efficiency since 
it did not have the c++ related issues with OMP.} 
\textcolor{red}{The parallel efficiency for the 
stand alone model has been reported in the supplemental materials section. 
The stand alone model was able to achieve an efficincey of 0.289 at 
128 cores. Again since the coupled model is built on the same framework it is very 
likely that this same performance could be achieved using the coupled model with 
the appropriate modifications and using a different cluster. However as stated 
earlier since the coupled PBM was so much faster than the DEM simulations it 
was not necessary for this work to further enhance the efficiency of the 
coupled PBM, which even with threading issues performed well.}  
...
d\textsubscript{50} plots of the mono-sized and BSD respectively. It can be seen that both these plots 
...
simulation compared to the BSD simulation is the time taken to simulate the DEM. It can be seen from 
Figure \ref{fig:rslts_DEM_timing_studies} that the 2mm mono-sized particle took about 1.6 times less 
than the BSD simulation time for the DEM. Since the physics of both the systems are not different, 
