...
cores. The time values indicate that dividing the
...
The extra slicing in the x-direction decreases the amount of computation carried out by 
a single process but it increases the number of communiations between the MPI slave and master processes. 
Thus, leading to slower simulations when the number of slices increase beyond 16 in the x-direction. 
...
in either the y-axis or the z-axis the simulation time increases. 
...
\centering
\includegraphics[scale=0.5]{rslsts_2mm_timing_mtlb.pdf}
\caption{}
\label{fig:rslts_DEM_2mm_timing}
\end{subfigure}\hfill
\begin{subfigure}{.45\textwidth}
\centering
\includegraphics[scale=0.5]{rslsts_2mm_DEM_speedup_mtlb.pdf}
\caption{}
...
cores were increased from 1 16 when compared to speed improvements obtained with a core count greater 
